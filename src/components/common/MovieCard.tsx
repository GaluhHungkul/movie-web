...
const MovieCard = ({ movie, className, isMovie } : { movie : TypeMovie, className? : string, isMovie? : boolean }) => {
  ...
  return (
    <Link onClick={handleToDetailMovieClick} href={`/${isMovie ? "movies" : "tv"}/detail/${movie.id}`} className={`group overflow-hidden aspect-[2/3] relative rounded group ${className}`} key={movie.id}>
